...
public class ZIPFileReaderTest extends TestCase {
    ...
    {
       // "calgary/
        Date.UTC(2013-1900,07-1,12,14,35,0),
       // "calgary/bib"
        Date.UTC(1991-1900,07-1,15,05,55,0),
      // "calgary/book1"
      ...
      // "calgary/paper6"
        Date.UTC(1991-1900,07-1,15,05,56,0),
      // "calgary/pic"
        Date.UTC(1991-1900,07-1,15,05,57,0),
      // "calgary/progc"
        Date.UTC(1991-1900,07-1,15,05,57,0),
      // "calgary/progl"
        Date.UTC(1991-1900,07-1,15,05,57,0),
      ...
      // "calgary/trans"
        Date.UTC(1991-1900,07-1,15,05,57,0),
      //"test\u00e9.txt"
        Date.UTC(2013-1900,07-1,12,14,39,0),
      // "\u81ea\u8ff0.txt"
        Date.UTC(2013-1900,07-1,12,14,38,0)
    };
      ...
      boolean useTimezone) throws IllegalArgumentException
  {
    ...
    public static boolean compareDates(Calendar zmtime, Calendar vdate)
    {
        if (zmtime.get(Calendar.YEAR)!=vdate.get(Calendar.YEAR))
            return false;
        if (zmtime.get(Calendar.MONTH)!=vdate.get(Calendar.MONTH))
            return false;
        if (zmtime.get(Calendar.DAY_OF_MONTH)!=vdate.get(Calendar.DAY_OF_MONTH))
            return false;
        if (zmtime.get(Calendar.HOUR_OF_DAY)!=vdate.get(Calendar.HOUR_OF_DAY))
            return false;
        if (zmtime.get(Calendar.MINUTE)!=vdate.get(Calendar.MINUTE))
            return false;
        return true;
    }
    ...
    public void testReadEntries() throws Exception
    {
        ...
        for (i = 0; i < ENTRIES_COUNT; i++)
        {
            ...
            cal.clear();
            cal.setTimeInMillis(ENTRIES_MTIME[i]);
            assertEquals(compareDates(entry.getModificationTime(),cal),true);
        }
        ...
        for (i = 1; i < ENTRIES_COUNT-1; i++)
        {
            ...
            {
               assertEquals(entry.getComment(),"This is a comment");
            }
        }
